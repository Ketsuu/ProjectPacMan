...
import com.gcstudios.entities.Maca;
		...
		try {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					}else if(pixelAtual == 0xFF7D0000) {
						Maca maca = new Maca(xx*16,yy*16,16,16,0,Entity.MACA_SPRITE);
						Game.entities.add(maca);
						Game.maca_contagem++;
					}else if(pixelAtual == 0xFF00CEBF){
						//CIANO
						Enemy enemy = new Enemy(xx*16,yy*16,16,16,1,Entity.ENEMY1);
						Game.entities.add(enemy);
					}else if(pixelAtual == 0xFFE83535){
						//ENEMY 2
						Enemy enemy = new Enemy(xx*16,yy*16,16,16,1,Entity.ENEMY2);
						Game.entities.add(enemy);
					}else if(pixelAtual == 0xFF116F24){
						//VERDE
						Enemy enemy = new Enemy(xx*16,yy*16,16,16,1,Entity.ENEMY3);
						Game.entities.add(enemy);
					}else if(pixelAtual == 0xFFB50076){
						//ENEMY 4
